    ...
    def execute(self):  # noqa: C901 'Command.handle' is too complex
        ...
        for basis_of_rent_row in peruste_rows:
            ...
            if basis_of_rent_row["KK"] and basis_of_rent_row["VUOSI"]:
                try:
                    index = Index.objects.get(
                        month=basis_of_rent_row["KK"], year=basis_of_rent_row["VUOSI"]
                    )
                except Index.DoesNotExist:
                    self.stdout.write(
                        "Index VUOSI {} KK {} does not exist!".format(
                            basis_of_rent_row["VUOSI"], basis_of_rent_row["KK"]
                        )
                    )
            ...
            (
                ...
                created,
            ) = BasisOfRentPropertyIdentifier.objects.get_or_create(
            ...
            for decision_column_prefix in decision_column_prefixes:
                decision_maker_string = basis_of_rent_row[
                    "{}_PAATTAJA".format(decision_column_prefix)
                ]
                decision_datetime = basis_of_rent_row[
                    "{}_PAATOSPVM".format(decision_column_prefix)
                ]
                decision_section_string = basis_of_rent_row[
                    "{}_PYKALA".format(decision_column_prefix)
                ]

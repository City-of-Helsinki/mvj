...
class LeaseholdTransferImportLog(TimeStampedSafeDeleteModel):
    ...
    class Meta:
        ...
        verbose_name_plural = pgettext_lazy("Model name", "Leasehold transfer import logs")
...
class LeaseholdTransfer(TimeStampedSafeDeleteModel):
    ...
    institution_identifier = models.CharField(verbose_name=_("Institution identifier"), max_length=127)
...
class LeaseholdTransferProperty(models.Model):
    ...
    transfer = models.ForeignKey(
        LeaseholdTransfer, verbose_name=_('Leasehold transfer'),
        on_delete=models.CASCADE, related_name='properties')

    class Meta:
        verbose_name = pgettext_lazy("Model name", "Leasehold transfer property")
        verbose_name_plural = pgettext_lazy("Model name", "Leasehold transfer properties")
...
class LeaseholdTransferParty(NameModel):
    ...
    transfer = models.ForeignKey(
        LeaseholdTransfer, verbose_name=_('Leasehold transfer'),
        on_delete=models.CASCADE, related_name='parties')

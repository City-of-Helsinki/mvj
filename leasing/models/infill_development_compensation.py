...
class InfillDevelopmentCompensation(TimeStampedSafeDeleteModel):
    ...
    class Meta:
        ...
        verbose_name_plural = pgettext_lazy("Model name", "Infill development compensations")

    def __str__(self):
        return self.name if self.name else 'InfillDevelopmentCompensation #{}'.format(self.id)
...
class InfillDevelopmentCompensationLease(TimeStampedSafeDeleteModel):
    ...
    infill_development_compensation = models.ForeignKey(
        InfillDevelopmentCompensation, verbose_name=_("Infill development compensation"),
        related_name='infill_development_compensation_leases', on_delete=models.PROTECT)

    lease = models.ForeignKey('leasing.Lease', verbose_name=_("Lease"),
                              related_name='infill_development_compensation_leases', on_delete=models.PROTECT)
    ...
    monetary_compensation_amount = models.DecimalField(verbose_name=_("Monetary compensation amount"), null=True,
                                                       blank=True, max_digits=10, decimal_places=2)

    # In Finnish: Korvausinvestointi
    compensation_investment_amount = models.DecimalField(verbose_name=_("Compensation investment amount"), null=True,
                                                         blank=True, max_digits=10, decimal_places=2)

    # In Finnish: Arvon nousu
    increase_in_value = models.DecimalField(verbose_name=_("Increase in value"), null=True, blank=True, max_digits=10,
                                            decimal_places=2)

    # In Finnish: Osuus arvon noususta
    part_of_the_increase_in_value = models.DecimalField(verbose_name=_("Part of the increase in value"), null=True,
                                                        blank=True, max_digits=10, decimal_places=2)

    # In Finnish: Vuokran alennus
    discount_in_rent = models.DecimalField(verbose_name=_("Discount in rent"), null=True, blank=True, max_digits=10,
                                           decimal_places=2)

    # In Finnish: Arvioitu maksuvuosi
    year = models.PositiveSmallIntegerField(verbose_name=_("Year"), null=True, blank=True)

    # In Finnish: Lähetetty SAP pvm
    sent_to_sap_date = models.DateField(verbose_name=_("Sent to SAP date"), null=True, blank=True)
...
class InfillDevelopmentCompensationDecision(TimeStampedSafeDeleteModel):
    ...
    class Meta:
        ...
        verbose_name_plural = pgettext_lazy("Model name", "Infill development compensation decisions")
...
class InfillDevelopmentCompensationIntendedUse(TimeStampedSafeDeleteModel):
    ...
    class Meta:
        ...
        verbose_name_plural = pgettext_lazy("Model name", "Infill development compensation intended uses")


def get_attachment_file_upload_to(instance, filename):
    return '/'.join(['idc_attachments', str(instance.infill_development_compensation_lease.id), filename])
...
class InfillDevelopmentCompensationAttachment(TimeStampedSafeDeleteModel):
    ...
    class Meta:
        ...
        verbose_name_plural = pgettext_lazy("Model name", "Infill development compensation attachments")

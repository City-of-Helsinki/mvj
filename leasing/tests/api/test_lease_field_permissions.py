...
def test_user_cant_view_any_fields(
...
@pytest.mark.django_db
def test_superuser_can_view_all_fields(
    django_db_setup, client, lease_test_data, user_factory
):
    user = user_factory(username="test_user")
    user.set_password("test_password")
    ...
    assert sorted(response.data.keys()) == sorted(
        LeaseRetrieveSerializer().get_fields().keys()
    )


@pytest.mark.django_db
def test_user_can_view_some_fields(
    django_db_setup, client, lease_test_data, user_factory
):
    user = user_factory(username="test_user")
    user.set_password("test_password")
...
@pytest.mark.django_db
def test_user_cannot_modify_field(
    django_db_setup, client, lease_test_data, user_factory
):
    user = user_factory(username="test_user")
    user.set_password("test_password")

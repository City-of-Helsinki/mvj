...
@pytest.mark.django_db
@pytest.mark.parametrize("field_permissions, expected_keys", [
    ([], []),
    (['view_contact_id'], ['id']),
...
def test_field_permission(django_db_setup, client, lease_test_data, user_factory, field_permissions, expected_keys):
    user = user_factory(username='test_user')
    user.set_password('test_password')
    user.save()

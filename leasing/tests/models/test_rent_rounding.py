...
def test_add_rounded_amount(django_db_setup, lease_factory, contact_factory, tenant_factory,
    ...
    tenant_rent_share_factory(tenant=tenant1, intended_use_id=2, share_numerator=1, share_denominator=2)
    ...
    tenant_contact_factory(type=TenantContactType.TENANT, tenant=tenant2, contact=contact2,
                           start_date=datetime.date(year=2000, month=1, day=1))
    tenant_rent_share_factory(tenant=tenant2, intended_use_id=2, share_numerator=1, share_denominator=2)

    rent = rent_factory(lease=lease, type=RentType.FIXED, cycle=RentCycle.JANUARY_TO_DECEMBER,
                        due_dates_type=DueDatesType.FIXED, due_dates_per_year=12)
    contract_rent_factory(rent=rent, intended_use_id=1, amount=1000, period=PeriodType.PER_YEAR, base_amount=1000,
                          base_amount_period=PeriodType.PER_YEAR)
    contract_rent_factory(rent=rent, intended_use_id=2, amount=100, period=PeriodType.PER_YEAR, base_amount=100,
                          base_amount_period=PeriodType.PER_YEAR)

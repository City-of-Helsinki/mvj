...
def test_get_latest_contract_returns_none_when_no_leasing_contracts(
    lease_factory, contract_factory
):
    lease = lease_factory()
    # Create contracts with different type (not LEASING_CONTRACT_TYPE_ID)
    contract_factory(
        lease=lease, type_id=LEASING_CONTRACT_TYPE_ID + 1, contract_number="2"
    )
    contract_factory(
        lease=lease, type_id=LEASING_CONTRACT_TYPE_ID + 2, contract_number="3"
    )
    result = _get_latest_contract(lease)
    assert result is None
...
def test_get_latest_contract_returns_none_when_contract_has_no_number(
    ...
    lease = lease_factory()
    contract_factory(
        lease=lease, type_id=LEASING_CONTRACT_TYPE_ID, contract_number=None
    )
    result = _get_latest_contract(lease)
    assert result is None
...
def test_get_latest_contract_returns_single_valid_contract(
    ...
    contract = contract_factory(
        lease=lease,
        type_id=LEASING_CONTRACT_TYPE_ID,
        contract_number="1",
...
def test_get_latest_contract_ignores_wrong_contract_types(
    ...
    contract_with_wrong_type = (  # noqa: F841 (unused variable by purpose)
        contract_factory(
            lease=lease,
            type_id=LEASING_CONTRACT_TYPE_ID + 1,
            contract_number="1",
            signing_date=datetime.date(2025, 1, 1),
        )
    )
...
def test_get_latest_contract_number_returns_empty_string_when_no_valid_contracts(
    ...
    lease = lease_factory()
    contract_factory(
        lease=lease, type_id=LEASING_CONTRACT_TYPE_ID + 1, contract_number="1"
    )
    result = get_latest_contract_number(lease)
    assert result == ""

...
class InspectionAttachmentCreateUpdateSerializer(FieldPermissionsSerializerMixin, serializers.ModelSerializer):
    id = serializers.IntegerField(required=False)
...
class InspectionSerializer(FieldPermissionsSerializerMixin, serializers.ModelSerializer):
    id = serializers.IntegerField(required=False)
    attachments = InspectionAttachmentSerializer(many=True, read_only=True, allow_null=True)

    class Meta:
        model = Inspection
        fields = ('id', 'inspector', 'supervision_date', 'supervised_date', 'description', 'attachments')

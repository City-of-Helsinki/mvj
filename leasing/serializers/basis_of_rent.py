...
class BasisOfRentDecisionSerializer(
...
class BasisOfRentDecisionCreateUpdateSerializer(
    FieldPermissionsSerializerMixin, serializers.ModelSerializer
):
    id = serializers.IntegerField(required=False)
...
class BasisOfRentRateSerializer(
...
class BasisOfRentRateCreateUpdateSerializer(
    EnumSupportSerializerMixin,
    FieldPermissionsSerializerMixin,
    serializers.ModelSerializer,
):
    ...
    build_permission_type = InstanceDictPrimaryKeyRelatedField(
        instance_class=BasisOfRentBuildPermissionType,
        queryset=BasisOfRentBuildPermissionType.objects.all(),
...
class BasisOfRentSerializer(
    FieldPermissionsSerializerMixin, serializers.ModelSerializer
):
    plot_type = BasisOfRentPlotTypeSerializer()
    rent_rates = BasisOfRentRateSerializer(many=True, required=False, allow_null=True)
    property_identifiers = BasisOfRentPropertyIdentifierSerializer(
        many=True, required=False, allow_null=True
    )
    decisions = BasisOfRentDecisionSerializer(
        many=True, required=False, allow_null=True
    )
...
class BasisOfRentCreateUpdateSerializer(
    UpdateNestedMixin, FieldPermissionsSerializerMixin, serializers.ModelSerializer
):
    plot_type = InstanceDictPrimaryKeyRelatedField(
        instance_class=BasisOfRentPlotType,
    ...
    decisions = BasisOfRentDecisionCreateUpdateSerializer(
        many=True, required=False, allow_null=True
    )

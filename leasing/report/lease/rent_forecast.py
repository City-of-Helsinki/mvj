...
INTERNAL_LEASE_TYPES = ['K0', 'Y0', 'Y1', 'Y2', 'Y3', 'Y4', 'Y5', 'Y6', 'Y7', 'Y8', 'Y9']
...
class RentForecastReport(AsyncReportBase):
    ...
    slug = 'rent_forecast'
    input_fields = {
        'start_year': forms.IntegerField(label=_('Start year'), required=True, min_value=2020, max_value=2050),
        'end_year': forms.IntegerField(label=_('End year'), required=True, min_value=2021, max_value=2050),
    }
    output_fields = {
        'lease_type': {
            'label': _('Lease type'),
    ...
    def get_data(self, input_data):  # NOQA C901 'RentForecastReport.get_data' is too complex
        ...
        for lease in leases:
            for year in years:
                try:
                    ...
                    rent_sums[rent_sums_key][year][lease.type.identifier] += rent_amount.get_total_amount()
        ...
        for year in years:
            ...
            for rent_sums_key in ['internal', 'external']:
                ...
                row_count = 0
                for lease_type, total_rent_amount in sorted(rent_sums[rent_sums_key][year].items()):
                    result.append({
                        'lease_type': lease_type,
                        'year': year,
                        'rent': Decimal(total_rent_amount).quantize(Decimal('.01'), rounding=ROUND_HALF_UP),
                    })
                    row_count += 1
                    data_row_num += 1
                ...
                total_row = ExcelRow([
                    ExcelCell(
                        column=0,
                        value=str(_('Total {year} {internal_external_text}').format(
                            year=year,
                            internal_external_text=internal_external_text
                        )),
                        format_type=FormatType.BOLD
                    ),
                    PreviousRowsSumCell(column=2, count=row_count, format_type=FormatType.BOLD_MONEY),
                ])
            ...
            year_total_row.cells.append(
                ExcelCell(column=0, value='{} {}'.format(str(_('Total')), year), format_type=FormatType.BOLD)
            )

            year_total_rent_sum_cell = SumCell(column=2, format_type=FormatType.BOLD_MONEY)
            for totals_row_num in totals_row_nums:
                year_total_rent_sum_cell.add_target_range((totals_row_num, 2, totals_row_num, 2))

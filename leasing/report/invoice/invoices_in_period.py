...
class InvoicesInPeriodReport(ReportBase):
    ...
    input_fields = {
        ...
        'lease_type': forms.ModelChoiceField(label=_('Laskutuslaji'), queryset=LeaseType.objects.all(),
                                             empty_label=None, required=False),
        'invoice_state': forms.ChoiceField(label=_('Invoice state'), required=False, choices=InvoiceState.choices())
    }
    output_fields = {
        'number': {
            'label': _('Number'),
    ...
    def get_response(self, request):
        ...
        totals_row.cells.append(SumCell(column=5, target_ranges=[(0, 5, invoice_count - 1, 5)]))
        totals_row.cells.append(SumCell(column=6, target_ranges=[(0, 6, invoice_count - 1, 6)]))
        totals_row.cells.append(SumCell(column=7, target_ranges=[(0, 7, invoice_count - 1, 7)]))

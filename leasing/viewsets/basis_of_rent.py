...
from leasing.serializers.basis_of_rent import (
    BasisOfRentCreateUpdateSerializer,
...
class BasisOfRentViewSet(
    ...
    search_fields = (
        ...
        "note",
        "property_identifiers__identifier",
    )
    ordering_fields = (
        "start_date",
        "end_date",
    ...
    def get_queryset(self):
        queryset = BasisOfRent.objects.select_related(
        ...
        if search_form.is_valid():
            ...
            if search_form.cleaned_data.get("reference_number"):
                queryset = queryset.filter(
                    decisions__reference_number__icontains=search_form.cleaned_data.get(
                        "reference_number"
                    )
                )

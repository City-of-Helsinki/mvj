...
The authoritative description of behavior is in the other documents in the
`spec` directory.
...
tasotarkastus procedure. This means the rent amount needs to be re-calculated in
2045. See
`leasing.models.rent.Rent.old_dwellings_in_housing_companies_price_index`.
...
`leasing.management.commands.import_periodica_rent_adjustment_index`.
...
Rent object automatically when the rent is created, if it uses tasotarkistus.

...
def test_urls(path):
    ...
    sc = res.status_code
    if sc == 200 :
        assert ct.startswith('text/html;'), f"Got {ct} response # curl -L {url}"
        # We are viewing internal pages so trusting the title should be adequate
        titles = html.fromstring(res.content).xpath('//title/text()')
        assert titles[0].encode('latin1').decode('utf-8') == loginpagetitle, f"Got non-login {ct} response # curl -L {url}"
    else:
        assert res.status_code == 401 or res.status_code == 404 , f"Got unexpected retcode {res.status_code} # curl -L -v {url}"

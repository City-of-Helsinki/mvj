...
SELECT lease.id AS vuokraus_id,
    ...
    concat(ltype.identifier, ( SELECT leasing_municipality.identifier
          ...
          WHERE leasing_municipality.id = lid.municipality_id), ( SELECT leasing_district.identifier
    ...
    ( SELECT leasing_payablerent.amount
           FROM leasing_payablerent
          WHERE (leasing_payablerent.rent_id IN ( SELECT leasing_rent.id
                   FROM leasing_rent
                  WHERE leasing_rent.lease_id = lease.id))
          ORDER BY leasing_payablerent.start_date DESC
         LIMIT 1) AS perittava_vuokra_summa,
    ( SELECT concat(to_char(leasing_payablerent.start_date::timestamp with time zone, 'fmDD.fmMM.YYYY'::text), ' - ', to_char(leasing_payablerent.end_date::timestamp with time zone, 'fmDD.fmMM.YYYY'::text)) AS concat
           FROM leasing_payablerent
          WHERE (leasing_payablerent.rent_id IN ( SELECT leasing_rent.id
                   FROM leasing_rent
                  WHERE leasing_rent.lease_id = lease.id))
  ...
  WHERE lease.deleted IS NULL AND l_area.archived_at IS NULL AND (lease.end_date IS NULL OR lease.end_date > now());

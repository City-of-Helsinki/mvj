...
def test_one_primary_address_in_leasearea(django_db_setup, lease_factory, rent_factory, contact_factory,
                                          tenant_factory, tenant_rent_share_factory, tenant_contact_factory,
                                          invoice_factory, invoice_row_factory, lease_area_factory,
    ...
    lease_area = lease_area_factory(lease=lease, identifier=get_random_string(), area=1000, section_area=1000)
    lease_area_address_factory(lease_area=lease_area, is_primary=True, address='Ensisijainen testiosoite',
                               postal_code='00550', city='Helsinki')
...
@pytest.mark.django_db
def test_one_nonprimary_address_in_leasearea(django_db_setup, lease_factory, rent_factory, contact_factory,
                                             tenant_factory, tenant_rent_share_factory, tenant_contact_factory,
                                             invoice_factory, invoice_row_factory, lease_area_factory,
                                             lease_area_address_factory):
    lease = lease_factory(
    ...
    lease_area = lease_area_factory(lease=lease, identifier=get_random_string(), area=1000, section_area=1000)
    lease_area_address_factory(lease_area=lease_area, is_primary=False, address='Ainoa ei-ensisijainen osoite',
                               postal_code='00550', city='Helsinki')
...
@pytest.mark.django_db
def test_one_primary_and_nonprimary_addresses_in_leasearea(django_db_setup, lease_factory, rent_factory,
                                                           contact_factory, tenant_factory, tenant_rent_share_factory,
                                                           tenant_contact_factory, invoice_factory, invoice_row_factory,
                                                           lease_area_factory, lease_area_address_factory):
    lease = lease_factory(
        type_id=1,
        municipality_id=1,
        district_id=5,
    ...
    lease_area = lease_area_factory(lease=lease, identifier=get_random_string(), area=1000, section_area=1000)
    lease_area_address_factory(lease_area=lease_area, is_primary=False, address='Jokutoinen osoite',
                               postal_code='00550', city='Helsinki')
    lease_area_address_factory(lease_area=lease_area, is_primary=True, address='Ensisijainen testiosoite',
                               postal_code='00550', city='Helsinki')
    lease_area_address_factory(lease_area=lease_area, is_primary=False, address='Toissijainen osoite',
                               postal_code='00550', city='Helsinki')
